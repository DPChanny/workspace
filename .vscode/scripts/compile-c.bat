...
if "%~3"=="" (
    echo [!] Usage: compile-c.bat [source_abs] [temp_abs] [relative_file]
    pause
    exit /b 1
)
...
set "OUTFILE=%TEMPROOT%/%RELFILE:.c=%%EXT%"
...
echo [BUILD] gcc %COMPILER_FLAGS% "%SOURCE%" -o "%OUTFILE%"
gcc %COMPILER_FLAGS% "%SOURCE%" -o "%OUTFILE%"
