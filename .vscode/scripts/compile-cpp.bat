...
if "%~3"=="" (
    echo [!] Usage: compile-cpp.bat [source_abs] [temp_abs] [relative_file]
    pause
    exit /b 1
)
...
set "OUTFILE=%TEMPROOT%/%RELFILE:.cpp=%%EXT%"
...
echo [BUILD] g++ %COMPILER_FLAGS% "%SOURCE%" -o "%OUTFILE%"
g++ %COMPILER_FLAGS% "%SOURCE%" -o "%OUTFILE%"
