...
static bool check_position(Vector _position) {
  return _position.y + 1 > 0 && _position.y < y_size && _position.x + 1 > 0 &&
         _position.x < x_size && map[_position.y][_position.x] != WALL;
}

static bool check_fire_position(Vector _position) {
  return check_position(_position) && map[_position.y][_position.x] != FIRE;
}

static bool check_body_position(Vector _position) {
  return check_fire_position(_position) &&
  ...
  while (!next_body_positions.empty()) {
    ...
    for (int i(0); i < queue_size; i++) {
      ...
      if (check_fire_position({position_buffer.y + 1, position_buffer.x}))
        next_fire_positions.push({position_buffer.y + 1, position_buffer.x});
      if (check_fire_position({position_buffer.y - 1, position_buffer.x}))
        next_fire_positions.push({position_buffer.y - 1, position_buffer.x});
      if (check_fire_position({position_buffer.y, position_buffer.x + 1}))
        next_fire_positions.push({position_buffer.y, position_buffer.x + 1});
      if (check_fire_position({position_buffer.y, position_buffer.x - 1}))
        next_fire_positions.push({position_buffer.y, position_buffer.x - 1});
    }
    ...
    for (int i(0); i < queue_size; i++) {
      ...
      if (position_buffer.y + 1 == y_size || position_buffer.x + 1 == x_size ||
          position_buffer.y == 0 || position_buffer.x == 0) {
        flag = true;
        break;
      }

      if (check_body_position({position_buffer.y + 1, position_buffer.x}))
        next_body_positions.push({position_buffer.y + 1, position_buffer.x});
      if (check_body_position({position_buffer.y - 1, position_buffer.x}))
        next_body_positions.push({position_buffer.y - 1, position_buffer.x});
      if (check_body_position({position_buffer.y, position_buffer.x + 1}))
        next_body_positions.push({position_buffer.y, position_buffer.x + 1});
      if (check_body_position({position_buffer.y, position_buffer.x - 1}))
        next_body_positions.push({position_buffer.y, position_buffer.x - 1});
    }
  }
